...
export async function getTaskList() {
    ...
    while (true) {
        const response = await fetch(`${KLING_CONFIG.baseUrl}?pageNum=${pageNum}&pageSize=${pageSize}`, {
            method: 'GET'
        });
        if (!response.ok) throw new Error('작업 목록 조회 실패');
        const data = await response.json();
        const tasks = data.data?.tasks || (Array.isArray(data.data) ? data.data : []);
...
export async function getTaskById(taskId) {
    const response = await fetch(`${KLING_CONFIG.baseUrl}/${taskId}`, {
        method: 'GET'
    });
    if (!response.ok) throw new Error('작업 조회 실패');
    return await response.json();
}
...
export async function getLipSyncTaskList() {
    ...
    while (true) {
        const response = await fetch(`${KLING_CONFIG.lipSyncUrl}?pageNum=${pageNum}&pageSize=${pageSize}`, {
            method: 'GET'
        });
